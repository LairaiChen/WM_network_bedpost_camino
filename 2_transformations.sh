...
if [ ! -e transform/dti_to_t1.mat ]; then    
    ...
    flirt -in T1/t1_brain_unbiased.nii.gz -ref /media/shulab/dxy_xuanwu/Lairai/templates/MNI152_T1_1mm_brain.nii -omat transform/t1_to_mni_affine.mat
fi

if [ ! -e transform/t1_to_mni_nonlinear_warp.nii.gz ]; then    
    # bedpost requires bvecs, bvals, data, nodif_brain_mask
    fnirt --ref=/media/shulab/dxy_xuanwu/Lairai/templates/MNI152_T1_1mm_brain.nii --in=T1/t1_brain_unbiased.nii.gz --aff=transform/t1_to_mni_affine.mat --cout=transform/t1_to_mni_nonlinear_warp
...
if [ ! -e DTI/bna_in_dti.nii.gz ]; then    
    applywarp --ref=T1/t1_brain_unbiased.nii --in=/media/shulab/dxy_xuanwu/Lairai/templates/BN_Atlas_246_1mm.nii --warp=transform/mni_to_t1_nonlinear_warp.nii.gz --out=T1/bna_in_t1.nii.gz --interp=nn
    applywarp --ref=T1/t1_brain_unbiased.nii --in=/media/shulab/dxy_xuanwu/Lairai/templates/aal116_1MM.nii --warp=transform/mni_to_t1_nonlinear_warp.nii.gz --out=T1/aal_in_t1.nii.gz --interp=nn
